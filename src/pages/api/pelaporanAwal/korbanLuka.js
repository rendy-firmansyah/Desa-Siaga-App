...
export default async function korbanMeninggalHandler(req, res) {
  if (req.method === "POST") {
    const { namaFaskes, jenisRawat, lakiLaki, wanita, jumlah, pelaporan_id } =
      req.body;
    ...
    try {
    const pelaporan = await prisma.korbanLuka.create({
      data: {
        namaFaskes,
        jenisRawat,
    ...
    return res
      .status(200)
      .json({ message: "Berhasil Menambahkan data Korban Luka - Luka", status : 'success'});
    }
    catch (error) {
  ...
  if (req.method === "GET") {
    try {
      ...
      const pelaporan = await prisma.korbanLuka.findMany({where : {pelaporan_id : parseInt(id)} });
