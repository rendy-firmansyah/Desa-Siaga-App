...
export default async function korbanMeninggalHandler(req,res){
    if(req.method === 'POST'){ 
        const {lokasi, gangguanAnak, gangguanDewasa, lakiLaki, perempuan, jumlah, bayi, balita, bumil, buteki, cacatLaki, cacatPerempuan, lansiaLaki, lansiaPerempuan, pelaporan_id} = req.body;
        if(!lokasi || !gangguanAnak || !gangguanDewasa || !lakiLaki || !perempuan || !jumlah || !bayi || !balita || !bumil || !buteki || !cacatLaki || !cacatPerempuan || !lansiaLaki || !lansiaPerempuan || !pelaporan_id){
        ...
        try{
            ...
            return res.status(200).json({message:"Berhasil menambahkan data korban pengungsi", status : "success"});
        }catch(error){
   ...
   if (req.method   === "GET") {
        try{
            ...
            const pengungsi = await prisma.korbanPengungsi.findMany({where:{pelaporan_id:parseInt(id)}});
